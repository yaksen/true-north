    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        {topProjects.map(summary => (
            <div key={summary.project.id} className="flex items-center">
                <Avatar className="h-9 w-9">
                    <AvatarFallback>{getInitials(summary.project.name)}</AvatarFallback>
                </Avatar>
                <div className="ml-4 space-y-1">
                    <p className="text-sm font-medium leading-none">{summary.project.name}</p>
                    <p className="text-sm text-muted-foreground">
                        Profit/Loss: {formatCurrency(summary.profitLoss, summary.project.currency)}
                    </p>
                </div>
                <div className="ml-auto font-medium">{formatCurrency(summary.totalIncome, summary.project.currency)}</div>
          </div>
        ))}
