...
import type { Invoice, Project, Lead, Discount } from '@/lib/types';
...
export function InvoiceForm({ invoice, projects, leads, closeForm }: InvoiceFormProps) {
  ...
  const form = useForm<InvoiceFormValues>({
    ...
    defaultValues: invoice ? {
      ...
      lineItems: [{ id: uuidv4(), description: '', quantity: 1, priceLKR: 0, priceUSD: 0 }],
      discounts: [],
      taxRate: 0,
      notes: '',
    },
        ...
        <div className="space-y-2">
            ...
            {lineItemFields.map((field, index) => (
                <div key={field.id} className="flex items-center gap-2">
                    <FormField control={form.control} name={`lineItems.${index}.description`} render={({ field }) => (<Input placeholder="Description" {...field} className="flex-grow" />)} />
                    <FormField control={form.control} name={`lineItems.${index}.quantity`} render={({ field }) => (<Input type="number" placeholder="Qty" {...field} className="w-20" />)} />
                    <FormField control={form.control} name={`lineItems.${index}.priceLKR`} render={({ field }) => (<Input type="number" placeholder="Price (LKR)" {...field} className="w-28" />)} />
                    <Button type="button" variant="ghost" size="icon" onClick={() => removeLineItem(index)}><Trash2 className="h-4 w-4 text-destructive" /></Button>
                </div>
            ))}
            <Button type="button" size="sm" variant="outline" onClick={() => appendLineItem({ id: uuidv4(), description: '', quantity: 1, priceLKR: 0, priceUSD: 0 })}>
