...
export function InvoiceView({ invoice, project, lead }: InvoiceViewProps) {
  ...
  const { user } = useAuth();
  const totals = calculateInvoiceTotals(invoice.lineItems, invoice.discounts, invoice.taxRate);
  ...
  const generatePDF = () => {
    ...
    const tableData = invoice.lineItems.map(item => [
        item.description,
        item.quantity,
        formatCurrency(project.currency === 'LKR' ? item.priceLKR : item.priceUSD),
        formatCurrency(project.currency === 'LKR' ? item.priceLKR * item.quantity : item.priceUSD * item.quantity),
    ]);
    ...
    // Totals
    const isLKR = project.currency === 'LKR';
    let currentY = finalY + 10;
    doc.setFontSize(10);
    doc.text("Subtotal:", 150, currentY);
    doc.text(formatCurrency(isLKR ? totals.subtotalLKR : totals.subtotalUSD), 190, currentY, { align: 'right' });

    if (totals.totalDiscountLKR > 0) {
        currentY += 6;
        doc.text("Discount:", 150, currentY);
        doc.text(`-${formatCurrency(isLKR ? totals.totalDiscountLKR : totals.totalDiscountUSD)}`, 190, currentY, { align: 'right' });
    }
    
    if (totals.taxAmountLKR > 0) {
        currentY += 6;
        doc.text(`Tax (${invoice.taxRate}%):`, 150, currentY);
        doc.text(formatCurrency(isLKR ? totals.taxAmountLKR : totals.taxAmountUSD), 190, currentY, { align: 'right' });
    }
    ...
    doc.text(formatCurrency(isLKR ? totals.totalLKR : totals.totalUSD), 190, currentY, { align: 'right' });
                    ...
                    <TableBody>
                        {invoice.lineItems.map(item => (
                            <TableRow key={item.id}>
                                ...
                                <TableCell className="text-right">{formatCurrency(project.currency === 'LKR' ? item.priceLKR : item.priceUSD)}</TableCell>
                                <TableCell className="text-right">{formatCurrency(project.currency === 'LKR' ? item.priceLKR * item.quantity : item.priceUSD * item.quantity)}</TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
                </Table>
                ...
                <div className="flex justify-end">
                    <div className="w-full max-w-sm space-y-2">
                        <div className="flex justify-between">
                            <span>Subtotal</span>
                            <span>{formatCurrency(project.currency === 'LKR' ? totals.subtotalLKR : totals.subtotalUSD)}</span>
                        </div>
                        ...
                        {invoice.taxRate > 0 && (
                             <div className="flex justify-between">
                                ...
                                <span>{formatCurrency(project.currency === 'LKR' ? totals.taxAmountLKR : totals.taxAmountUSD)}</span>
                            </div>
                        )}
                        <Separator />
                        <div className="flex justify-between font-bold text-lg">
                            <span>Total</span>
                            <span>{formatCurrency(project.currency === 'LKR' ? totals.totalLKR : totals.totalUSD)}</span>
                        </div>
                    </div>
                </div>
                {invoice.notes && (
