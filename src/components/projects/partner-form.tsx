...
export function PartnerForm({ partner, projectId, channels, closeForm }: PartnerFormProps) {
  ...
  const handleExtractDetails = async () => {
    ...
    try {
        ...
        const extractedData: ExtractPartnerDetailsOutput = await extractPartnerDetails({
            prompt: aiPrompt,
            imageDataUri: imageDataUri
        });

        if (extractedData.name) form.setValue('name', extractedData.name);
        if (extractedData.roleInProject) form.setValue('roleInProject', extractedData.roleInProject);
        ...
        console.error('Error extracting partner details:', error);
        toast({
            variant: 'destructive',
        ...
        <Card className='h-fit'>
            ...
            <CardContent className="space-y-4">
                <div className="space-y-2">
                    ...
                    <Textarea
                        id="ai-prompt"
                        placeholder="Paste partner info..."
                        value={aiPrompt}
