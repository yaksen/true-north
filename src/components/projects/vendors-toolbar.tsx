...
import { findVendor, type FindVendorOutput } from '@/ai/flows/find-vendor-flow';

interface VendorsToolbarProps {
  onFilterChange: (filters: { serviceType: string; search: string }) => void;
}

export function VendorsToolbar({ onFilterChange }: VendorsToolbarProps) {
  ...
  }, [searchTerm, serviceTypeFilter, onFilterChange]);
  ...
  const handleAiSearch = async () => {
    ...
    try {
        const result: FindVendorOutput = await findVendor({ prompt: searchTerm });
        
        setServiceTypeFilter(result.serviceType || '');
