...
import { findPartner, type FindPartnerOutput } from '@/ai/flows/find-partner-flow';

interface PartnersToolbarProps {
  onFilterChange: (filters: { role: string; search: string }) => void;
}

export function PartnersToolbar({ onFilterChange }: PartnersToolbarProps) {
  ...
  }, [searchTerm, roleFilter, onFilterChange]);
  ...
  const handleAiSearch = async () => {
    ...
    try {
        const result: FindPartnerOutput = await findPartner({ prompt: searchTerm });
        
        setRoleFilter(result.role || '');
